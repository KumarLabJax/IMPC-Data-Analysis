#This file takes the already transformed data from the previous python scripts and uses it to generate plots 
...
#Sets the variables for the directory as well as the procedure code
...
#Produces a time series plot based on a singular phenotype and type of time factor (such as date)
...
#Produces a time series plot for every phenotype and phenotyping center using the date of experiment as a time factor
...
#Produces an allele plot for every phenotype and phenotyping center
...
#Produces a Z score plot for every phenotype and phenotyping center
...
#Produces a timseries plot for a singular phenotype using the time of day as the time factor
...
#Produces a time day series plot for every phenotype and phenotyping center
...
produceAllPlots <- function(procedure_code, directory, pipeline = "IMPC"){
  ...
  produceAllMultiTimeSeries(procedure_code,directory,isControl=T,isLine=T, pipeline=pipeline)
  produceAllMultiTimeSeries(procedure_code,directory,isControl=F,isLine=T, pipeline=pipeline)
  ...
  #HISTDENSITY
  print(paste("STARTED DENSITYHIST PLOTS: ", procedure_code, sep=""))
  produceAllHistDensity <- function(procedure_code, directory, isControl = T, pipeline = "IMPC") 
  produceAllHistDensity <- function(procedure_code, directory, isControl = F, pipeline = "IMPC") 
  print(paste("FINISHED DENSITYHIST PLOTS: ", procedure_code, sep=""))
